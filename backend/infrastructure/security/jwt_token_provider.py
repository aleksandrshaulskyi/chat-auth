...
from application.exceptions import InvalidJWTException
from application.ports.jwt_token_provider import JWTTokenProviderPort

from settings import settings


class JWTTokenProvider(JWTTokenProviderPort):

    async def issue_pair(self, user_id: int) -> dict:
        access_token_exp_time = datetime.now(timezone.utc) + timedelta(minutes=8)
